...
class TextView(QuietTextView):
    ...
    def end(self, score, duration):
        ...
        if score.all_mutants:
            self.level_print(
                "killed: {} ({:.1f}%)".format(
                    score.killed_mutants, 100 * score.killed_mutants / score.all_mutants
                ),
                2,
            )
            self.level_print(
    ...
    def cant_load(self, name, exception):
        self.level_print(
            ...
            + "{} ({}: {})".format(name, exception.__class__.__name__, exception)
        )
    ...
    def incompetent(self, time, *args, **kwargs):
        self.level_print(
            self.time_format(time) + " " + self.decorate("incompetent", "cyan"),
            continuation=True,
        )
...
class AccReportView:
    ...
    def mutation(self, number, mutations, module, mutant):
        mutations = [
            {"operator": mutation.operator.name(), "lineno": mutation.node.lineno}
            for mutation in mutations
        ]
    ...
    def end_mutation(
        self, status, time=None, killer=None, tests_run=None, exception_traceback=None
    ):
...
class HTMLReportView(AccReportView):
    ...
    def end_mutation(self, *args, **kwargs):
        ...
        file_path = os.path.join(
            self.dir_name, "mutants", "{}.html".format(self.current_mutation["number"])
        )
