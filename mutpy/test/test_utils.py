...
class ModulesLoaderTest(unittest.TestCase):
    ...
    def assert_module(self, module_object, module_name, module_path, attrs):
        ...
        self.assertMultiLineEqual(
            os.path.abspath(module_object.__name__), os.path.abspath(module_name)
        )
    ...
    def test_bad_target_class(self):
        self.assertRaises(
            utils.ModulesLoaderException,
            lambda: self.loader.load_single("a.b.c.sample.Y"),
        )

    def test_bad_target_method(self):
        self.assertRaises(
            utils.ModulesLoaderException,
            lambda: self.loader.load_single("a.b.c.sample.X.g"),
        )

    def test_bad_module(self):
        self.assertRaises(
            utils.ModulesLoaderException,
            lambda: self.loader.load_single("a.b.c.example"),
        )
...
class GetByPythonVersionTest(unittest.TestCase):
    ...
    def test_get_proper_class(self):
        cls = utils.get_by_python_version(
            classes=[self.A, self.B], python_version=(3, 1)
        )
    ...
    def test_get_lower_class(self):
        cls = utils.get_by_python_version(
            classes=[self.A, self.B], python_version=(3, 3)
        )
...
class InjectImporterTest(unittest.TestCase):
    def test_inject(self):
        ...
        eval(
            compile(target_module_content, "target.py", "exec"), target_module.__dict__
        )
        importer.install()
