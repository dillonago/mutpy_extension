...
class BaseTestCases:
    ...
    class BaseTestRunnerTest(unittest.TestCase):
        ...
        def setUp(self):
            if None in [
                self.TEST_RUNNER_CLS,
                self.TEST_SRC_SUCCESS,
                self.TEST_SRC_FAIL,
                self.TEST_SRC_SKIP,
            ]:
                self.fail(
                    "Subclasses must override TEST_RUNNER_CLS, TEST_SRC_SUCCESS, TEST_SRC_FAIL and TEST_SRC_SKIP"
                )

        def run_test(self, target_src, test_src):
            with FileMockModulesLoader(
                "target", target_src
            ) as target_loader, FileMockModulesLoader("test", test_src) as test_loader:
                target_loader.load()
                runner = self.TEST_RUNNER_CLS(
                    test_loader, 5, utils.StdoutManager(True), False
                )
